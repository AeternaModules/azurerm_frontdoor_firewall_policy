...
EOT
  ...
  validation {
    condition = alltrue([
      for k, v in var.frontdoor_firewall_policies : (
        v.custom_block_response_status_code == null || (contains([200, 403, 405, 406, 429], v.custom_block_response_status_code))
      )
    ])
    error_message = "must be one of: 200, 403, 405, 406, 429"
  }
  validation {
    condition = alltrue([
      for k, v in var.frontdoor_firewall_policies : (
